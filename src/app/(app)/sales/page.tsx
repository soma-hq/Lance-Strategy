		...
		return (
			<div
				...
				<p style={{ fontSize: "0.88rem", textAlign: "center" }}>
					Ajoutez des produits et un vendeur
					<br />
					pour afficher l&apos;aperçu de la facture
				</p>
			</div>
		);
	}
				...
				<div
					...
					<div
						style={{
							...
							color: "#fff",
							display: "grid",
							gridTemplateColumns: "1fr 40px 80px 80px",
							gap: "0.5rem",
					...
					<div
						...
						<div
							...
							<div
								...
								<span
									className={`badge ${SaleStatusBadges[sale.status]}`}
...
export default function SalesPage() {
	...
	const [lines, setLines] = useState<LineItem[]>([
		{ id: uid(), productId: "", quantity: 1, unitPriceTTC: 0 },
	]);
	...
	const load = useCallback(async () => {
		...
		try {
			...
			const data = await api.get<{ sales: Sale[]; total: number }>(
				`/sales?${params}`,
			);
			setSales(data.sales);
			setTotal(data.total);
	...
	const linesTotal = lines.reduce(
		(s, l) => s + l.quantity * l.unitPriceTTC,
		0,
	);
			...
			prev.map((l) => {
				...
				if (field === "productId") {
					...
					return {
						...
						unitPriceTTC: product?.price ?? l.unitPriceTTC,
					};
				}
	...
	const handleSave = async () => {
		...
		try {
			if (editMode && editTarget) {
				...
				showToast(
					`${created.length} vente${created.length > 1 ? "s" : ""} créée${created.length > 1 ? "s" : ""}`,
					"success",
				);
			...
			<div className="logs-controls" style={{ marginBottom: "1rem" }}>
				<div
					...
					<input
						...
						type="search"
						placeholder="Produit, client, vendeur…"
						value={search}
				...
				<select
					...
					{SalePlatforms.filter(Boolean).map((p) => (
							...
							<tbody>
								{filtered.map((s) => (
									...
									<tr
										...
										<td>
											<span
												className={`badge ${SaleStatusBadges[s.status]}`}
												style={{ fontSize: "0.72rem" }}>
					...
					<div
						...
						{hasCreated && (
							<div
								...
								{createdSales.length} vente
								{createdSales.length > 1 ? "s" : ""} créée
								{createdSales.length > 1 ? "s" : ""} avec succès
							</div>
						)}
						...
						<div className="modal-footer">
							...
							{hasCreated ? (
								...
								<button
									...
									title={
										hasStockError
											? "Un ou plusieurs produits ont un stock insuffisant"
											: undefined
									}>
									{saving
										? "Enregistrement…"
