			...
			<div
				...
				<div
					style={{
						...
						height: "80px",
						minWidth: "80px",
						background: "linear-gradient(135deg, #2e4a8a, #b8923a)",
						borderRadius: "16px",
						display: "flex",
						alignItems: "center",
						justifyContent: "center",
					...
					<div
						...
						<div className="stat-card">
							<div className="stat-info">
								...
								<div className="stat-label">Taches actives</div>
							</div>
						</div>
						<div className="stat-card">
							<div className="stat-info">
					...
					{data.logs.slice(0, 8).map((log) => (
						<div
							...
							<div
								...
								{log.module && (
									<span
										className={`badge ${ModuleBadges[log.module] || "badge-light"}`}
							...
							.map((ev) => (
								<div
									...
									<div
										...
										<span
											className={`badge ${EventTypeBadges[ev.type] || "badge-light"}`}
											style={{ fontSize: "0.65rem" }}>
	...
	return (
		<div>
			...
			<div
				...
				<div className="stat-card">
					<div className="stat-info">
						<div className="stat-value">{data.salesTotalCount}</div>
						...
						<tbody>
							{data.sales.map((s) => (
								<tr key={s.id}>
									...
									<td>
										<span
											className={`badge ${SaleStatusBadges[s.status] || "badge-light"}`}
											style={{ fontSize: "0.7rem" }}>
...
function TabClients({ data, sales }: { data: ProfileData; sales: Sale[] }) {
							...
							{data.clients.map((c) => (
								<tr key={c.id}>
									...
									<td
										...
										{eur(clientRevenueMap.get(c.id) || 0)}
									</td>
								</tr>
							))}
						</tbody>
...
function TabTaches({ data }: { data: ProfileData }) {
	...
	const todoTasks = data.tasks.filter((t) => t.status === "en_attente");
						...
						return (
							<div
								...
								<div
									...
									<span
										className={`badge ${ProjectStatusBadges[p.status] || "badge-light"}`}
										style={{ fontSize: "0.68rem" }}>
		...
		<div>
			...
			{data.events.length === 0 ? (
				...
				<>
					...
					{past.length > 0 && (
						<>
							<SectionTitle>Passes ({past.length})</SectionTitle>
							<div
								style={{
									display: "flex",
									flexDirection: "column",
	...
	return (
		<div>
			<div
				...
				total
				{data.logsTotal > 50 ? ` (50 dernieres affichees)` : ""}
			</div>
