...
export async function GET(request: NextRequest) {
	try {
		...
		chartStart.setDate(
			chartStart.getDate() - (DashboardConfig.chartLookbackDays - 1),
		);
		...
		const [
			...
			// Recent sales with minimal includes
			prisma.sale.findMany({
				take: DashboardConfig.recentSales,
				orderBy: { date: "desc" },
				include: {
			...
			prisma.task.findMany({
				...
				take: DashboardConfig.overdueTasks,
				include: {
					assignee: {
			...
			// Top products by revenue via groupBy
			prisma.sale.groupBy({
				by: ["productId"],
				_sum: { totalTTC: true },
				where: { status: "completed" },
				orderBy: { _sum: { totalTTC: "desc" } },
				take: DashboardConfig.topProducts,
			}),

			// Recent logs
			prisma.log.findMany({
				take: DashboardConfig.recentLogs,
			...
			prisma.event.findMany({
				where: { date: { gte: now } },
				orderBy: { date: "asc" },
				take: DashboardConfig.upcomingEvents,
		...
		const dailyRevenue = Array.from(
			{ length: DashboardConfig.chartLookbackDays },
			(_, i) => {
