...
import { DefaultValues } from "@/types/constants";
...
export const GET = withAuth("view", async (request) => {
	...
	const page = Math.max(
		...
		parseInt(searchParams.get("page") || String(ApiConfig.defaultPage)),
	);
	const limit = Math.min(
		...
		Math.max(
			1,
			parseInt(
				searchParams.get("limit") || String(ApiConfig.defaultLimit),
			),
		),
	);
...
export const POST = withAuth("create", async (request, currentUser) => {
	...
	const sale = await prisma.sale.create({
		data: {
			...
			status: status || DefaultValues.sale.status,
			platform: platform || DefaultValues.sale.platform,
	...
	if (sale.status === "completed" && sale.seller) {
		...
		await prisma.userStock.upsert({
			where: {
				productId_username: { productId: sale.productId, username },
			},
