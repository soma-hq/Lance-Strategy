 ...
 * @returns Bar element
 */
function RankingBar({ percent, color = "var(--accent)" }: { percent: number; color?: string }) {
	return (
		<div style={{ height: "6px", background: "var(--bg-alt)", borderRadius: "3px", overflow: "hidden", minWidth: "60px" }}>
			<div
				style={{
					height: "100%",
...
function BarChart({
	...
	const computedWidth =
		totalBars > 0 ? Math.min(barMaxWidth, Math.max(barMinWidth, Math.floor(800 / totalBars) - barGap)) : 12;
		...
		<div style={{ position: "relative" }}>
			...
			<div
				...
				{[1, 0.75, 0.5, 0.25, 0].map((frac) => (
					<span key={frac} style={{ fontSize: "0.68rem", color: "var(--text-muted)", textAlign: "right", paddingRight: "6px" }}>
						{maxVal * frac >= 1000 ? `${((maxVal * frac) / 1000).toFixed(0)}k` : (maxVal * frac).toFixed(0)}
					</span>
				))}
			</div>
				...
				{data.map((d, i) => {
					...
					return (
						<div
							...
							onMouseEnter={(e) => {
								setHoveredIdx(i);
								onBarEnter(e, d.fullLabel, formatNumber(d.value));
							}}
							onMouseLeave={() => {
								setHoveredIdx(null);
								onBarLeave();
			...
			{/* X axis labels */}
			<div style={{ marginLeft: "52px", display: "flex", gap: `${barGap}px`, marginTop: "4px" }}>
				{data.map((d, i) => (
					<div
						key={i}
						style={{
 ...
 */
function DonutChart({ segments, centerLabel = "total" }: { segments: DonutSegment[]; centerLabel?: string }) {
		...
		<div style={{ display: "flex", alignItems: "center", gap: "2rem" }}>
			...
			<div
				...
				<div
					...
					<div style={{ fontSize: "1.1rem", fontWeight: 700, color: "var(--text)" }}>{total}</div>
					<div style={{ fontSize: "0.6rem", color: "var(--text-muted)" }}>{centerLabel}</div>
				</div>
			</div>
			...
			<div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
				{segments.map((seg) => (
					<div key={seg.label} style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
						...
						<div>
							<div style={{ fontSize: "0.8rem", fontWeight: 600, color: "var(--text)" }}>{seg.label}</div>
...
function DayHeatmap({ data, label }: { data: DayPoint[]; label: string }) {
	...
	return (
		<div>
			<div style={{ fontSize: "0.75rem", fontWeight: 600, marginBottom: "0.5rem", color: "var(--text)" }}>{label}</div>
					...
					<div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "3px" }}>
						<div style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
							<span
								style={{
									...
									height: "18px",
									borderRadius: "4px",
									background: idx < 3 ? "var(--accent)" : "var(--bg-alt)",
						...
						<div style={{ textAlign: "right" }}>
							{renderRight ? (
								...
								<>
									<div style={{ fontSize: "0.78rem", fontWeight: 700 }}>{formatNumber(row.count)}</div>
									<div style={{ fontSize: "0.65rem", color: "var(--text-muted)" }}>{row.share.toFixed(1)}&nbsp;%</div>
								</>
							)}
						</div>
					</div>
					<RankingBar percent={(row.count / Math.max(maxCount, 1)) * 100} color={barColor} />
				</div>
			))}
		</div>
	);
	...
	return (
		<div
			...
			<div
				...
				style={{ background: "var(--card-bg)", borderRadius: "12px", padding: "1.5rem", minWidth: "340px", maxWidth: "600px", width: "90%", maxHeight: "80vh", overflow: "auto", boxShadow: "0 8px 32px rgba(0,0,0,0.25)" }}>
				{/* Modal header */}
...
export default function AnalyticsPage() {
	...
	const load = useCallback(async () => {
		...
		try {
			...
			const [salesData, clientsData, tasksData, projectsData, productsData, usersData] = await Promise.all([
	...
	const sales = useMemo(
		() => allSales.filter((s) => { const d = new Date(s.date); return d >= periodFrom && d < periodTo; }),
	...
	const postponedSales = useMemo(() => sales.filter((s) => s.status === "postponed"), [sales]);

	// Clients created in period
	const newClients = useMemo(
		() => allClients.filter((c) => { const d = new Date(c.createdAt); return d >= periodFrom && d < periodTo; }),
		[allClients, periodFrom, periodTo]
	);

	// Active tasks
	const activeTasks = useMemo(() => allTasks.filter((t) => t.status === "en_cours" || t.status === "en_attente"), [allTasks]);
	...
	const productRanking = useMemo((): RankingRow[] => {
		...
		const totalUnits = completedSales.reduce((sum, s) => sum + s.quantity, 0) || 1;
		return Array.from(map.values())
	...
	const salesChartData = useMemo((): BarDataPoint[] => {
		...
		if (daysInPeriod > 90) {
			...
			return Array.from(monthMap.entries()).map(([m, value]) => {
				...
				return {
					label: `${mo}/${y.slice(2)}`,
					fullLabel: new Date(Number(y), Number(mo) - 1, 1).toLocaleDateString("fr-FR", { month: "long", year: "numeric" }),
					value,
				};
			});
		}
		...
		return Array.from(dailyMap.entries()).map(([date, value]) => ({
			label: new Date(date).toLocaleDateString("fr-FR", { day: "2-digit", month: "2-digit" }),
			fullLabel: new Date(date).toLocaleDateString("fr-FR", { weekday: "long", day: "numeric", month: "long" }),
			value,
		}));
	...
	const clientsMonthlyData = useMemo((): BarDataPoint[] => {
		...
		return Array.from({ length: 6 }, (_, i) => {
			...
			return {
				...
				fullLabel: monthDate.toLocaleDateString("fr-FR", { month: "long", year: "numeric" }),
				value: count,
			};
		});
	}, [allClients]);
	...
	const tasksByUser = useMemo((): RankingRow[] => {
		...
		return Array.from(map.entries())
			.map(([name, count]) => ({ name, count, share: (count / maxCount) * 100 }))
			.sort((a, b) => b.count - a.count);
	}, [allTasks]);
	...
	const saleStatusSegments = useMemo((): DonutSegment[] => {
		const total = sales.length || 1;
		return [
			{ label: "Complétées", count: completedSales.length, color: "var(--success)", percent: (completedSales.length / total) * 100 },
			{ label: "Reportées", count: postponedSales.length, color: "var(--warning)", percent: (postponedSales.length / total) * 100 },
			{ label: "Annulées", count: cancelledSales.length, color: "var(--danger)", percent: (cancelledSales.length / total) * 100 },
		];
	...
	// Busiest day
	const busiestDayIdx = dayOfWeekData.reduce((best, d) => (d.value > dayOfWeekData[best].value ? d.day : best), 0);
	...
	function renderModalContent(modal: StatModalState) {
		if (modal.type === "sales") {
			...
			return (
				<div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
					...
					{saleItems.map((s) => (
						<div key={s.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.6rem 0.75rem", background: "var(--bg-alt)", borderRadius: "8px" }}>
							<div>
								<div style={{ fontSize: "0.82rem", fontWeight: 600 }}>{s.product?.name ?? `Produit #${s.productId}`}</div>
								<div style={{ fontSize: "0.72rem", color: "var(--text-muted)" }}>
									{s.client?.name ?? "Client inconnu"} &middot; {s.seller?.name ?? "—"}
								</div>
							</div>
							<div style={{ textAlign: "right" }}>
								<div style={{ fontSize: "0.82rem", fontWeight: 700 }}>{s.quantity} unité(s)</div>
								<span className={`badge badge-${s.status === "completed" ? "success" : s.status === "cancelled" ? "danger" : "warning"}`} style={{ fontSize: "0.65rem" }}>
									{s.status === "completed" ? "Complétée" : s.status === "cancelled" ? "Annulée" : "Reportée"}
								</span>
							</div>
						</div>
					))}
		...
		if (modal.type === "tasks_active" || modal.type === "tasks_overdue") {
			...
			return (
				<div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
					{taskItems.length === 0 && (
						<div style={{ color: "var(--text-muted)", fontSize: "0.85rem", textAlign: "center", padding: "1rem" }}>Aucune tâche</div>
					)}
					{taskItems.map((t) => (
						<div key={t.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.6rem 0.75rem", background: "var(--bg-alt)", borderRadius: "8px" }}>
							<div>
							...
							<span className={`badge badge-${t.status === "en_cours" ? "warning" : t.status === "bloquee" ? "danger" : "light"}`} style={{ fontSize: "0.65rem" }}>
		...
		if (modal.type === "projects_active") {
			...
			return (
				<div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
					{projItems.length === 0 && (
						<div style={{ color: "var(--text-muted)", fontSize: "0.85rem", textAlign: "center", padding: "1rem" }}>Aucun projet</div>
					)}
					{projItems.map((p) => {
						...
						return (
							<div key={p.id} style={{ padding: "0.6rem 0.75rem", background: "var(--bg-alt)", borderRadius: "8px" }}>
								<div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
									<div style={{ fontSize: "0.82rem", fontWeight: 600 }}>
										{p.emoji && `${p.emoji} `}{p.name}
									</div>
									<span style={{ fontSize: "0.72rem", fontWeight: 700, color: "var(--accent)" }}>{pct}%</span>
								</div>
								<RankingBar percent={pct} color="var(--accent)" />
								<div style={{ fontSize: "0.68rem", color: "var(--text-muted)", marginTop: "4px" }}>{done}/{total} tâches</div>
							</div>
						);
					})}
				</div>
			...
			return (
				<div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
					{clientItems.length === 0 && (
						<div style={{ color: "var(--text-muted)", fontSize: "0.85rem", textAlign: "center", padding: "1rem" }}>Aucun nouveau client</div>
					)}
					{clientItems.map((c) => (
						<div key={c.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.6rem 0.75rem", background: "var(--bg-alt)", borderRadius: "8px" }}>
							<div>
								<div style={{ fontSize: "0.82rem", fontWeight: 600 }}>{c.name}</div>
								<div style={{ fontSize: "0.72rem", color: "var(--text-muted)" }}>{c.company ?? c.email ?? "—"}</div>
							</div>
		...
		if (modal.type === "products_alert") {
			const prodItems = modal.items as (Product & { totalStock: number })[];
			return (
				<div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
					{prodItems.length === 0 && (
						<div style={{ color: "var(--text-muted)", fontSize: "0.85rem", textAlign: "center", padding: "1rem" }}>Aucun produit en alerte</div>
					)}
					{prodItems.map((p) => (
						<div key={p.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.6rem 0.75rem", background: "var(--bg-alt)", borderRadius: "8px" }}>
							<div>
								<div style={{ fontSize: "0.82rem", fontWeight: 600 }}>{p.name}</div>
								<div style={{ fontSize: "0.72rem", color: "var(--text-muted)" }}>Seuil : {p.alertThreshold}</div>
							</div>
							<div style={{ textAlign: "right" }}>
								<div style={{ fontSize: "0.82rem", fontWeight: 700, color: p.totalStock === 0 ? "var(--danger)" : "var(--warning)" }}>
	...
	return (
		<div>
			...
			<div className="page-header-box">
				<div className="phb-left">
					...
					<div className="phb-text">
						<h1>Analytiques</h1>
						<p className="page-description">Performance opérationnelle et activité</p>
					</div>
				</div>

				<div className="page-header-box-actions" style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
					{/* Period selector */}
					<div style={{ display: "flex", gap: "0.25rem", background: "var(--bg-alt)", borderRadius: "10px", padding: "0.25rem", flexWrap: "wrap" }}>
						{(Object.keys(PERIOD_LABELS) as PeriodKey[]).map((key) => (
							<button
								...
								style={{
									...
									borderRadius: "7px",
									border: "none",
									background: period === key ? "var(--card-bg)" : "transparent",
			...
			{/* Custom date picker */}
			{period === "custom" && (
				<div className="card" style={{ marginBottom: "1rem", display: "flex", alignItems: "center", gap: "1rem", padding: "0.75rem 1.15rem" }}>
					<span style={{ fontSize: "0.82rem", fontWeight: 600, color: "var(--text)" }}>Période personnalisée :</span>
					<div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
						<label style={{ fontSize: "0.78rem", color: "var(--text-muted)" }}>Du</label>
						<input
							type="date"
							value={customFrom}
							onChange={(e) => setCustomFrom(e.target.value)}
							style={{ padding: "0.35rem 0.55rem", borderRadius: "6px", border: "1px solid var(--border-color)", background: "var(--bg-alt)", color: "var(--text)", fontSize: "0.8rem" }}
						/>
						<label style={{ fontSize: "0.78rem", color: "var(--text-muted)" }}>au</label>
						<input
							type="date"
							value={customTo}
				...
				<>
					...
					<Section
						...
						<div className="stats-grid" style={{ gridTemplateColumns: "repeat(5, 1fr)" }}>
							...
							<StatCard
								...
								color="var(--accent)"
								onClick={() => openModal("sales", "Total ventes — détail", sales)}
							/>
							...
							<StatCard
								...
								color="#8b5cf6"
								onClick={() => openModal("clients_new", "Nouveaux clients", newClients)}
							/>
							...
							<StatCard
								...
								color="var(--warning)"
								onClick={() => openModal("tasks_active", "Tâches actives", activeTasks)}
							/>
							...
							<StatCard
								...
								color="var(--success)"
								onClick={() => openModal("projects_active", "Projets actifs", activeProjects)}
							/>
							...
							<StatCard
								...
								onClick={() => openModal(
									...
									alertProducts.map((p) => ({
										...p,
										totalStock: p.stocks?.reduce((s, st) => s + st.quantity, 0) ?? 0,
									}))
								)}
							/>
						</div>
					...
					<Section
						...
						<div className="stats-grid" style={{ gridTemplateColumns: "repeat(3, 1fr)", marginBottom: "1.5rem" }}>
							<StatCard
								...
								color="var(--success)"
								onClick={() => openModal("sales", "Ventes complétées", completedSales)}
							/>
							<StatCard
								...
								color="var(--warning)"
								onClick={() => openModal("sales", "Ventes reportées", postponedSales)}
							/>
							<StatCard
								label="Ventes annulées"
								value={formatNumber(cancelledSales.length)}
								sub={sales.length > 0 ? `${((cancelledSales.length / sales.length) * 100).toFixed(1)}% du total` : "—"}
								color="var(--danger)"
								onClick={() => openModal("sales", "Ventes annulées", cancelledSales)}
							/>
						</div>
						...
						<div style={{ marginBottom: "1.5rem" }}>
							...
							{salesChartData.length === 0 || sales.length === 0 ? (
								...
								<BarChart
									data={salesChartData}
									height={200}
									onBarEnter={(e, label, value) => setTooltip({ x: e.clientX, y: e.clientY - 50, label, value: `${value} vente(s)` })}
						...
						<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
							<div>
								<div style={{ fontSize: "0.82rem", fontWeight: 600, color: "var(--text)", marginBottom: "0.75rem" }}>Top vendeurs (nb ventes)</div>
								<RankingList
									rows={sellerRanking}
									...
									renderRight={(row) => (
										<>
											<div style={{ fontSize: "0.78rem", fontWeight: 700 }}>{formatNumber(row.count)} ventes</div>
											<div style={{ fontSize: "0.65rem", color: "var(--text-muted)" }}>{row.share.toFixed(1)}&nbsp;%</div>
										</>
									)}
								/>
							...
							<div>
								<div style={{ fontSize: "0.82rem", fontWeight: 600, color: "var(--text)", marginBottom: "0.75rem" }}>Par plateforme</div>
								<RankingList
						...
						<div style={{ marginTop: "1.5rem" }}>
							<div style={{ fontSize: "0.82rem", fontWeight: 600, color: "var(--text)", marginBottom: "0.75rem" }}>Répartition par statut</div>
							<DonutChart segments={saleStatusSegments} centerLabel="ventes" />
						</div>

						{/* Busiest day insight */}
						{sales.length > 0 && (
							<div style={{ marginTop: "1.5rem" }}>
								<DayHeatmap data={dayOfWeekData} label="Ventes par jour de la semaine" />
								<div style={{ fontSize: "0.72rem", color: "var(--text-muted)", marginTop: "0.5rem" }}>
									Jour le plus actif : <strong style={{ color: "var(--accent)" }}>{DAY_NAMES_FULL_FR[busiestDayIdx]}</strong>
								</div>
							</div>
						)}
					</Section>
						...
						<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
							...
							<div>
								...
								<BarChart
									...
									height={160}
									onBarEnter={(e, label, value) => setTooltip({ x: e.clientX, y: e.clientY - 50, label, value: `${value} nouveau(x)` })}
									onBarLeave={() => setTooltip(null)}
								/>
							</div>
							...
							<div>
								...
								<DonutChart segments={clientStatusSegments} centerLabel="clients" />
							</div>
						</div>
					</Section>
					...
					<Section
						...
						<div className="stats-grid" style={{ gridTemplateColumns: "repeat(3, 1fr)", marginBottom: "1.5rem" }}>
							...
							<StatCard
								...
								color="var(--danger)"
								onClick={() => openModal("tasks_overdue", "Tâches en retard", overdueTasks)}
							/>
							<StatCard
								label="Utilisateurs actifs"
								value={formatNumber(allUsers.filter((u) => u.active).length)}
						...
						<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
							<div>
								...
								<DonutChart segments={taskStatusSegments} centerLabel="tâches" />
							</div>
							<div>
								<div style={{ fontSize: "0.82rem", fontWeight: 600, color: "var(--text)", marginBottom: "0.75rem" }}>
									Tâches terminées par membre — {PERIOD_LABELS[period]}
								</div>
								<RankingList
									rows={completedTasksByUser.slice(0, 8)}
									maxCount={completedTasksByUser[0]?.count ?? 1}
									barColor="var(--success)"
									renderRight={(row) => (
										<div style={{ fontSize: "0.78rem", fontWeight: 700 }}>{formatNumber(row.count)}</div>
									)}
						...
						<div style={{ marginTop: "1.5rem" }}>
							...
							<RankingList
								...
								renderRight={(row) => (
									<>
										<div style={{ fontSize: "0.78rem", fontWeight: 700 }}>{formatNumber(row.count)} tâches</div>
										<div style={{ fontSize: "0.65rem", color: "var(--text-muted)" }}>{row.share.toFixed(1)}&nbsp;%</div>
									</>
								)}
							/>
					...
					<Section
						...
						<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
							...
							<div>
								...
								<DonutChart segments={projectStatusSegments} centerLabel="projets" />
							</div>
								...
								{allProjects.length === 0 ? (
									...
									<div style={{ display: "flex", flexDirection: "column", gap: "0.65rem" }}>
										{allProjects
											...
											.sort((a, b) => {
												const pctA = (a.taskCount ?? 0) > 0 ? ((a.completedTaskCount ?? 0) / (a.taskCount ?? 1)) * 100 : 0;
												const pctB = (b.taskCount ?? 0) > 0 ? ((b.completedTaskCount ?? 0) / (b.taskCount ?? 1)) * 100 : 0;
												return pctB - pctA;
											})
											.slice(0, 8)
											.map((p) => {
												...
												return (
													<div key={p.id}>
														<div style={{ display: "flex", justifyContent: "space-between", marginBottom: "3px" }}>
															<span style={{ fontSize: "0.8rem", fontWeight: 600, maxWidth: "160px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
																{p.emoji && `${p.emoji} `}{p.name}
															</span>
															<span style={{ fontSize: "0.78rem", fontWeight: 700, color: pct >= 75 ? "var(--success)" : pct >= 40 ? "var(--warning)" : "var(--danger)" }}>
																{pct}%
															</span>
														</div>
														<RankingBar
															percent={pct}
															color={pct >= 75 ? "var(--success)" : pct >= 40 ? "var(--warning)" : "var(--danger)"}
														/>
														<div style={{ fontSize: "0.65rem", color: "var(--text-muted)", marginTop: "2px" }}>{done}/{total} tâches</div>
													</div>
												);
											})}
									</div>
					...
					<Section
						...
						<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
							...
							<div>
								<div style={{ fontSize: "0.82rem", fontWeight: 600, color: "var(--text)", marginBottom: "0.75rem" }}>
									Produits les plus vendus (unités) — {PERIOD_LABELS[period]}
								</div>
								<RankingList
									rows={productRanking}
									...
									renderRight={(row) => (
										<>
											<div style={{ fontSize: "0.78rem", fontWeight: 700 }}>{formatNumber(row.count)} unités</div>
											<div style={{ fontSize: "0.65rem", color: "var(--text-muted)" }}>{row.share.toFixed(1)}&nbsp;%</div>
										</>
									)}
								/>
								...
								{alertProducts.length === 0 ? (
									...
									<div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
										{alertProducts.slice(0, 8).map((p) => {
											const totalStock = p.stocks?.reduce((s, st) => s + st.quantity, 0) ?? 0;
											const stockPct = p.alertThreshold > 0 ? Math.min((totalStock / p.alertThreshold) * 100, 100) : 0;
											return (
												<div key={p.id}>
													<div style={{ display: "flex", justifyContent: "space-between", marginBottom: "3px" }}>
														<span style={{ fontSize: "0.8rem", fontWeight: 600, maxWidth: "160px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
														...
														<span style={{ fontSize: "0.78rem", fontWeight: 700, color: totalStock === 0 ? "var(--danger)" : "var(--warning)" }}>
															{totalStock} / {p.alertThreshold}
														</span>
													</div>
													<RankingBar
														percent={stockPct}
														color={totalStock === 0 ? "var(--danger)" : "var(--warning)"}
													/>
												</div>
											);
										})}
										{alertProducts.length > 8 && (
											<button
												...
												style={{ fontSize: "0.78rem", padding: "0.35rem 0.75rem", marginTop: "0.25rem" }}
												onClick={() => openModal(
													"products_alert",
												...
												Voir tous ({alertProducts.length})
											</button>
										)}
									</div>
								)}
						...
						{completedSales.length > 0 && (
							<div style={{ marginTop: "1.5rem", padding: "1rem", background: "var(--bg-alt)", borderRadius: "8px" }}>
								<div style={{ fontSize: "0.78rem", fontWeight: 600, marginBottom: "0.5rem", color: "var(--text)" }}>
									Tendance des ventes — 7 derniers jours
								</div>
								<Sparkline
									values={Array.from({ length: 7 }, (_, i) => {
										const d = new Date(Date.now() - (6 - i) * 86400000).toISOString().slice(0, 10);
										return allSales.filter((s) => s.status === "completed" && new Date(s.date).toISOString().slice(0, 10) === d).length;
									})}
									color="var(--accent)"
								/>
			...
			{tooltip && (
				<div
					...
					<div style={{ fontSize: "0.68rem", fontWeight: 400, opacity: 0.75, marginTop: "2px" }}>{tooltip.label}</div>
				</div>
			)}

			{/* Stat detail modal */}
			{statModal && (
				<Modal title={statModal.title} onClose={() => setStatModal(null)}>
