...
function getSalesCount(client: Client): number {
	return (client.sales || []).filter((s: Sale) => s.status === "completed").length;
}
...
export default function ClientsPage() {
	...
	return (
		<div>
			...
			<div className="page-header-box">
				<div className="phb-left">
					...
					<div className="phb-text">
						<h1>Clients</h1>
						<p className="page-description">Gestion de la clientele</p>
					</div>
				</div>
				<div className="page-header-box-actions">
					<div style={{ display: "flex", gap: "0.35rem" }}>
						{(["table", "cards"] as const).map((m) => (
							<button
								key={m}
								className={viewMode === m ? "btn-secondary btn-sm" : "btn-icon"}
								onClick={() => setViewMode(m)}
								title={m === "table" ? "Vue tableau" : "Vue cartes"}
							>
			...
			>
				{[
					{ label: "Total clients", value: stats.total, icon: <IconUsers />, variant: "info" },
					{ label: "Nouveaux ce mois", value: stats.newThisMonth, icon: <IconCalendar />, variant: "warning" },
					{ label: "CA total clients", value: EUR(stats.caTotal), icon: <IconTrendingUp />, variant: "success" },
					{ label: "CA moyen / client", value: EUR(stats.avgRevenue), icon: <IconShoppingBag />, variant: "primary" },
				].map((s) => (
									...
									{filtered.map((c) => {
										...
										return (
											...
											>
												...
												<td>
													<span className={`badge ${STATUS_BADGE[c.status] || "badge-light"}`}>
												...
												<td style={{ fontSize: "0.82rem", color: "var(--text-secondary)" }}>
													{lastPurchase ? DATE_FR_SHORT(lastPurchase) : "---"}
												</td>
												<td>
								...
								>
									...
									>
										<div style={{ textAlign: "center" }}>
											...
											<div style={{ color: "var(--text-muted)", marginTop: "0.15rem" }}>CA total</div>
										</div>
										<div style={{ textAlign: "center" }}>
											<div style={{ fontWeight: 700, fontSize: "0.95rem", color: "var(--text)" }}>
												{salesCount}
											</div>
											<div style={{ color: "var(--text-muted)", marginTop: "0.15rem" }}>Achats</div>
										</div>
										<div style={{ textAlign: "center" }}>
											<div style={{ fontWeight: 700, fontSize: "0.82rem", color: "var(--text)" }}>
												{lastPurchase ? DATE_FR_SHORT(lastPurchase) : "---"}
											</div>
											<div style={{ color: "var(--text-muted)", marginTop: "0.15rem" }}>Dernier</div>
										</div>
									</div>
								</div>
							);
					...
					>
						...
						<div className="modal-header" style={{ flexShrink: 0 }}>
							<div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
								...
								<div>
									<h3 style={{ margin: 0, lineHeight: 1.2 }}>{selected.name}</h3>
									<div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginTop: "0.2rem" }}>
										<span style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>
											{selected.company || "Particulier"}
										</span>
										<span className={`badge ${STATUS_BADGE[selected.status] || "badge-light"}`}>
						...
						>
							{(
								[
									...
									{ key: "factures", label: "Factures", icon: <IconDownload /> },
									{ key: "historique", label: "Historique ventes", icon: <IconShoppingBag /> },
									{ key: "notes", label: "Notes", icon: <IconEdit /> },
								...
								<button
									...
									onClick={() => {
										...
										if (tab.key === "profil") setEditingProfile(false);
										if (tab.key === "notes") setNotesValue(selected.notes || "");
									}}
									style={{
										display: "flex",
										alignItems: "center",
										...
										color: detailTab === tab.key ? "var(--accent)" : "var(--text-muted)",
							...
							{detailTab === "profil" && !editingProfile && (
								<div>
									...
									>
										...
										>
											<div style={{ fontWeight: 700, fontSize: "1.15rem", color: "var(--success)" }}>
										...
										>
											<div style={{ fontWeight: 700, fontSize: "1.15rem", color: "var(--text)" }}>
												{getLastPurchaseDate(selected) ? DATE_FR_SHORT(getLastPurchaseDate(selected)!) : "---"}
											</div>
											<div style={{ fontSize: "0.75rem", color: "var(--text-muted)", marginTop: "0.25rem" }}>
												Dernier achat
											</div>
									...
									>
										{[
											{ icon: <IconUser />, label: "Nom complet", value: selected.name },
											{ icon: <IconBuilding />, label: "Societe", value: selected.company || "---" },
											{ icon: <IconMail />, label: "Email", value: selected.email || "---" },
											{ icon: <IconPhone />, label: "Telephone", value: selected.phone || "---" },
											{
												icon: <IconCalendar />,
												label: "Date de creation",
												value: DATE_FR(selected.createdAt),
											},
											{
												icon: <IconClock />,
												label: "Derniere interaction",
												value: getLastPurchaseDate(selected)
													? DATE_FR(getLastPurchaseDate(selected)!)
													: "Aucune vente",
											},
										].map((item, i) => (
											<div
								...
								<div>
									<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.85rem" }}>
										{[
											...
											{ key: "company", label: "Societe", type: "text", placeholder: "Nom de la societe" },
											{ key: "email", label: "Email", type: "email", placeholder: "email@exemple.com" },
											{ key: "phone", label: "Telephone", type: "tel", placeholder: "+33 6 00 00 00 00" },
									...
									<div style={{ display: "flex", gap: "0.5rem", marginTop: "1rem" }}>
										<button className="btn-primary btn-sm" onClick={handleSaveProfile} disabled={savingProfile}>
											<IconSave />
											{savingProfile ? "Enregistrement..." : "Enregistrer"}
										</button>
										<button className="btn-secondary btn-sm" onClick={() => setEditingProfile(false)}>
											Annuler
										</button>
									</div>
								</div>
								...
								<div>
									...
									>
										<div className="form-group" style={{ marginBottom: 0, flex: "0 0 auto" }}>
											<label style={{ fontSize: "0.75rem" }}>Date debut</label>
											<input
												type="date"
										...
										<div className="form-group" style={{ marginBottom: 0, flex: "0 0 auto" }}>
											<label style={{ fontSize: "0.75rem" }}>Date fin</label>
											<input
												type="date"
												value={invoiceDateTo}
									...
									>
										...
										>
											<div style={{ fontWeight: 700, color: "var(--success)", fontSize: "1rem" }}>
												{EUR(filteredSalesTotals.totalTTC)}
											</div>
											<div style={{ fontSize: "0.72rem", color: "var(--text-muted)" }}>
												Total TTC
											</div>
										...
										>
											<div style={{ fontWeight: 700, color: "var(--text-secondary)", fontSize: "1rem" }}>
												{EUR(filteredSalesTotals.totalHT)}
											</div>
											<div style={{ fontSize: "0.72rem", color: "var(--text-muted)" }}>
												Total HT
											</div>
									...
									{filteredSales.length === 0 ? (
										...
										>
											Aucune facture
											{(invoiceDateFrom || invoiceDateTo) ? " pour cette periode" : ""}
										</div>
									) : (
										<div style={{ overflowX: "auto" }}>
											<table className="data-table">
													...
													{filteredSales.map((s) => (
														<tr key={s.id}>
															<td style={{ fontSize: "0.82rem" }}>
																{DATE_FR_SHORT(s.date)}
															</td>
															<td>
																{(s as Sale & { product?: { name: string } }).product?.name || "---"}
															</td>
															...
															<td style={{ textAlign: "right" }}>
																{s.status === "completed" && (
																	<button
																		...
																		onClick={() => downloadInvoice(s as Sale)}
																	>
																		<IconDownload />
																	</button>
																)}
											...
											{salesTimeline.map((s, i) => {
												...
												return (
													<div
														...
														style={{
															position: "relative",
															marginBottom: i < salesTimeline.length - 1 ? "1.25rem" : 0,
															paddingBottom: i < salesTimeline.length - 1 ? "1.25rem" : 0,
															borderBottom:
																i < salesTimeline.length - 1
															...
															<div style={{ flex: 1 }}>
																...
																>
																	<span style={{ fontWeight: 600, fontSize: "0.9rem", color: "var(--text)" }}>
																		{(s as Sale & { product?: { name: string } }).product?.name || `Vente #${s.id}`}
																	</span>
																	<span
																		className={`badge badge-${
																			isCompleted ? "success" : isCancelled ? "danger" : "warning"
																		}`}
																	>
																		{SALE_STATUS_LABELS[s.status] || s.status}
																	</span>
																...
																<div style={{ display: "flex", gap: "1rem", fontSize: "0.8rem", color: "var(--text-muted)", flexWrap: "wrap" }}>
																	...
																	{(s as Sale & { seller?: { name: string } }).seller?.name && (
																		<span style={{ display: "flex", alignItems: "center", gap: "0.3rem" }}>
																			<IconUser />
																			{(s as Sale & { seller?: { name: string } }).seller?.name}
																		</span>
									...
									{salesTimeline.length > 0 && (
										...
										>
											<span style={{ fontSize: "0.85rem", fontWeight: 600, color: "var(--text-secondary)" }}>
												{salesTimeline.length} vente{salesTimeline.length > 1 ? "s" : ""} au total
											</span>
											<span style={{ fontWeight: 700, fontSize: "1.05rem", color: "var(--success)" }}>
												{EUR(getTotalCA(selected))} de CA
											</span>
										</div>
									)}
								</div>
									...
									<div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
										{canEdit ? (
											<button
												...
												disabled={savingNotes || notesValue === (selected.notes || "")}
											>
												<IconSave />
										...
										{notesValue !== (selected.notes || "") && (
											<span style={{ fontSize: "0.75rem", color: "var(--warning, #f39c12)", fontStyle: "italic" }}>
				...
				<div className="modal-overlay">
					<div className="modal-content" style={{ maxWidth: "520px" }}>
						...
						<div className="modal-footer">
							<button className="btn-secondary btn-sm" onClick={() => setShowFormModal(false)}>
								Annuler
							</button>
